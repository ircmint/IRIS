...
# ============================================================
# RIDER is passed as argv[1] (e.g. "Rider2_AZ") so one script serves all
# 4 riders without duplicated copies - each gets its own OUTPUT_DIR/cache
# so runs never collide or overwrite each other.
if len(sys.argv) < 2:
    raise SystemExit("Usage: python qwen2_riderX_7ep.py <RiderName e.g. Rider2_AZ>")
RIDER = sys.argv[1]
...
OUTPUT_DIR = fos.path.join(SCRATCH_ROOT, "qwen2vl_2b_{RIDER.lower()}_lora_out_7ep")
...
GRAD_ACCUM_STEPS = 4
LEARNING_RATE = 1e-4
# 7-epoch variant of qwen2_rider1.py (which used MAX_EPOCHS=3 with
# EARLY_STOP_PATIENCE=2 as a time-boxed scope reduction). This run trains
# for up to 7 epochs instead; early-stop patience widened proportionally
# so it doesn't stop right as longer-horizon improvement might appear.
# Distinct OUTPUT_DIR (qwen2vl_2b_rider1_lora_out_7ep) so the original
# 3-epoch adapter/eval_summary/predictions are not overwritten - both are
# preserved for a real before/after comparison.
MAX_EPOCHS = 7
