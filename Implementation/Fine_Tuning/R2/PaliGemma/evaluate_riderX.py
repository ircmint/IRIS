"""
evaluate_paligemma_riderX.py - parameterized version of
evaluate_paligemma_rider1.py. Evaluate the fine-tuned PaliGemma-3B LoRA
adapter for any rider on its held-out validation split. Mirrors the
Qwen evaluate_riderX_7ep.py metrics/structure, adapted for PaliGemma's
single-image input + suffix-based generation.

Usage: python evaluate_paligemma_riderX.py <RiderName e.g. Rider2_AZ>
"""
...
if len(sys.argv) < 2:
    raise SystemExit("Usage: python evaluate_paligemma_riderX.py <RiderName e.g. Rider2_AZ>")
RIDER = sys.argv[1]
...
def main():
    ...
    for i, row in val_df.iterrows():
        try:
            ...
            frame = sample_mid_frame(video_path, start, end)
            prompt_text = schema_instructions + "\n" + build_telemetry_summary(row)

            inputs = processor(
                images=frame, text=prompt_text, return_tensors="pt"
            ).to(model.device)
            prompt_len = inputs["input_ids"].shape[1]

            with torch.no_grad():
                gen_ids = model.generate(**inputs, max_new_tokens=MAX_NEW_TOKENS, do_sample=False)
            gen_ids_trimmed = gen_ids[:, prompt_len:]
    ...
    print(f"\n===== EVALUATION SUMMARY ({RIDER}, PaliGemma-3B) =====")
