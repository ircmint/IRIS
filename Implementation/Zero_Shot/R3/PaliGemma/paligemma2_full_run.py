"""
Full 40-clip x 3-condition PaliGemma2-3B-pt-224 run.
Control test (paligemma2_control_test.py / pg2_check2.py) verified:
  - Model is NOT degenerate: correctly answers control yes/no questions
    ("no" for outer-space/dinosaur/airplane controls) and produces varied,
    content-grounded open-ended captions per clip.
  - However on the actual evasive-maneuver yes/no question, the model
    answered "yes" on all 10 sampled control-test clips regardless of gold
    label (same collapse-to-yes behavior as PaliGemma-1, documented in
    paligemma_full_run.py by the other agent). Running the full 40-clip
    sweep anyway per this task's mandate to report real numbers even if
    degenerate/poor, not to hide them.
"""
...
def main():
    ...
    out_path = os.path.join(OUT_DIR, f"motor__paligemma2-3b__{args.condition}.json")
